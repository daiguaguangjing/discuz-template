...
<!--{if $modmenu['thread']}-->
	<div id="modmenu" class="zaobm bg0 p15 mb15">
		<!--{eval $modopt=0;}-->
		<!--{if $_G['forum']['ismoderator']}-->
...
<!--{hook/viewthread_beginline}-->
	...
	<div class="ct2">
		...
		<div {if $close_leftinfo}class="mn"{else}class="mn-sd"{/if}>
				...
				<table cellspacing="0" cellpadding="0">
					<tr>
						...
						<td class="plc ptm pbm vwthd">
							<!--{if !IS_ROBOT}-->
								<div class="y">
									<!--{if $post['invisible'] == 0}--><a href="forum.php?mod=viewthread&action=printable&tid=$_G[tid]" title="{lang thread_printable}" target="_blank"><i class="fico-print fic4 fc-s vm"></i></a><!--{/if}-->
									<!--{if !$close_leftinfo}-->
							...
							<h1 style="display:inline;">
								<span id="thread_subject" class="fic24">$_G[forum_thread][subject]</span>
								<!--{if $_G['forum_thread']['typeid'] && $_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']]}-->
									<!--{if !IS_ROBOT && ($_G['forum']['threadtypes']['listable'] || $_G['forum']['status'] == 3)}-->
									<a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=typeid&typeid=$_G[forum_thread][typeid]" class="xg2">[{$_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']]}]</a>
									<!--{else}-->
									<a class="xg2">[{$_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']]}]</a>
									<!--{/if}-->
								<!--{/if}-->
								<!--{if !empty($threadsorts) && $_G['forum_thread']['sortid'] && $_G['forum']['threadsorts']['types']['sortid']}-->
								<a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=sortid&sortid=$_G[forum_thread][sortid]" class="xg2">[{$_G['forum']['threadsorts']['types'][$_G['forum_thread']['sortid']]}]</a>
								<!--{/if}-->
							</h1>
							<span class="xg1">
		...
		<div class="sd">
			...
			<div class="vt_header_sd">
			<!--{loop $postlist $post}-->
				<!--{if $post['first']}-->
				<div class="top">
					<!--{if $_G['forum_thread']['authorid'] && $_G['forum_thread']['author']}-->
						<a href="home.php?mod=space&uid=$_G['forum_thread']['authorid']" target="_blank" title="$_G[forum_thread][author]">{echo avatar($post['authorid'], 'big')}</a>
						<a href="home.php?mod=space&uid=$_G['forum_thread']['authorid']" target="_blank" title="$_G[forum_thread][author]"><h1 class="fic15">$_G['forum_thread']['author']</h1></a>
					<!--{else}-->
						<a>{echo avatar($post['0'], 'big')}</a>
						<h1 class="fic15">{lang anonymous}</h1>
					<!--{/if}-->
				</div>
				<div class="info">
			...
			<!--{if $post['relateitem']}-->
			<div class="zaobm bg0 mb10 zaorelated">
				<div class="bm_h cl"><h2>{lang related_thread}</h2></div>
				<div class="bm_c cl">
					<ul class="cl">
						<!--{loop $post['relateitem'] $var}-->
						<li class="mb10"><a href="forum.php?mod=viewthread&tid=$var[tid]" title="$var[subject]" class="fic6" target="_blank">$var[subject]</a></li>
						<!--{/loop}-->
					</ul>
				</div>
			</div>
			...
			<div class="zaobm bg0 mb10">
				<div class="bm_c cl">
					<a href="forum.php?mod=redirect&goto=nextoldset&tid=$_G[tid]" title="{lang last_thread}" class="z xg2"><i class="fico-left fic4 xg2 vm"></i>{lang last_thread}</a>
					<a href="forum.php?mod=redirect&goto=nextnewset&tid=$_G[tid]" title="{lang next_thread}" class="y xg2">{lang next_thread}<i class="fico-right fic4 xg2 vm" style="margin-right:-0.2em;"></i></a>
				</div>
			</div>
			<!--{/if}-->
		</div>
