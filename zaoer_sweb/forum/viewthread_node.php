		...
		<div id="favatar$post[pid]" class="pls cl favatar">
			<!--{if $post['authorid'] && $post['username'] && !$post['anonymous']}-->
				<!--{if $_G['setting']['authoronleft']}-->
					<div class="pi">
						<div class="authi">
							<a href="home.php?mod=space&uid=$post['authorid']" target="_blank" class="xw1"{if $post[groupcolor]} style="color: $post[groupcolor]"{/if}>$post[author]</a>
							<span class="authorverifys">$authorverifys</span>
						</div>
					</div>
				<!--{/if}-->
		...
		<div class="pi"<!--{if $close_leftinfo && !$post['anonymous'] && $postshowavatars && $showavatars}--> style="height:48px"<!--{/if}-->>
			...
			<div class="pti">
				...
				<!--{if $close_leftinfo && !$post['anonymous'] && $postshowavatars && $showavatars}-->
					<a href="home.php?mod=space&uid=$post[authorid]" target="_blank" class="xi2 z vt_plc_avt" style="padding-right:5px;">{echo avatar($post['authorid'], 'small')}</a>
				<!--{/if}-->
				
				<!--{eval $_self = $thread['author'] && $post['author'] == $thread['author'] && $post['position'] !== '1';}-->
				<!--{if $_self }-->
					<em class="authicn fico-person fic4 fnmr vm" id="authicon$post[pid]" title="{lang thread_author}" style="display:none;"></em>
				<!--{else}-->
					<!--{if !$post['anonymous'] && $_G['cache']['groupicon'][$post['groupid']]}-->
						<img class="authicn vm" style="display:none;" id="authicon$post[pid]" src="$_G['cache']['groupicon'][$post['groupid']]" />
					<!--{else}-->
						<img class="authicn vm" style="display:none;" id="authicon$post[pid]" src="$_G['cache']['groupicon'][0]" />
					<!--{/if}-->
				<!--{/if}-->

				<!--{if $post['authorid'] && !$post['anonymous']}-->
					<!--{if $_self}-->
					<em class="rq">{lang thread_author}</em><span class="pipe">|</span>
					<!--{/if}-->
