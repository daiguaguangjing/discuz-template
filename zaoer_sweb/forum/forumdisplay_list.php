			...
			<tr>
				...
				<!--{if CURMODULE != 'guide'}-->
					<div class="tf">
						...
						<!--{else}-->
						<a{if empty($_G['cookie']['forumdefstyle'])} href="forum.php?mod=forumdisplay&fid=$_G[fid]&forumdefstyle=yes" class="chked y"{else} href="forum.php?mod=forumdisplay&fid=$_G[fid]&forumdefstyle=no" class="unchk y"{/if} title="{lang view_thread_imagemode}{lang view_thread}" style="margin-right:20px;">{lang view_thread_imagemode}</a>
						<!--{/if}-->
		...
		<form method="post" autocomplete="off" name="moderate" id="moderate" action="forum.php?mod=topicadmin&action=moderate&fid=$_G[fid]&infloat=yes&nopost=yes">
			...
			<table summary="forum_$_G[fid]" cellspacing="0" cellpadding="0" id="threadlisttableid">
				<!--{if (!$simplestyle || !$_G['forum']['allowside'] && $page == 1) && !empty($announcement)}-->
					<tbody>
						<tr>
							<td class="tl_avatar"><a href="home.php?mod=space&uid=$announcement['authorid']"><img src="./template/zaoer_sweb/zaoer/announcement.png" /></a></td>
							...
							<th style="width:100%;">
								...
								<div class="tl_txt cl">
									{lang announcement}:&nbsp;
									<a href="home.php?mod=space&uid=$announcement[authorid]" c="1">$announcement[author]</a>
									<span>&nbsp;&nbsp;$announcement[starttime]&nbsp;</span>
								</div>
							</th>
						</tr>
					</tbody>
				...
				<!--{if !$separatepos || !$_G['setting']['forumseparator']}-->
					<tbody id="separatorline" class="emptb" style="display:none;">
						<tr>
							<td></td>
							<!--{if !$_GET['archiveid'] && $_G['forum']['ismoderator']}--><td class="o"></td><!--{/if}-->
							<th></th>
						</tr>
					</tbody>
						...
						<!--{loop $_G['forum_threadlist'] $key $thread}-->							
							...
							<tbody id="separatorline" style="display:none;">
								<tr class="ts">
									...
									<th>
										<!--{if empty($_G['forum']['picstyle']) && $_GET['orderby'] == 'lastpost' && !$_GET['filter']}--><a href="javascript:;" onclick="checkForumnew_btn('{$_G['fid']}')" title="{lang showupgrade}" class="forumrefresh">{lang forum_thread}</a><!--{else}-->&nbsp;<!--{/if}-->
									</th>
								</tr>
							</tbody>
							<script type="text/javascript">hideStickThread();</script>
							<!--{/if}-->
							<!--{if $separatepos <= $key + 1}--><!--{ad/threadlist}--><!--{/if}-->
								...
								<tr>
									...
									<th>
										<div class="tl_tit cl">
											...
											<!--{/if}-->
											<!--{if $thread['weeknew']}-->
												<a href="forum.php?mod=redirect&tid=$thread[tid]&goto=lastpost#lastpost" class="rq">New</a>
											<!--{/if}-->
											<!--{if !$thread['forumstick'] && ($thread['isgroup'] == 1 || $thread['fid'] != $_G['fid'])}-->
												<!--{if $thread['related_group'] == 0 && $thread['closed'] > 1}--><!--{eval $thread['tid']=$thread['closed'];}--><!--{/if}-->
										...
										<div class="tl_txt cl">
											...
											<span class="y">
												<i>{lang reply}$thread[allreplies]</i>
					...
					<!--{else}-->
						...
						<ul id="waterfall" class="ml waterfall cl">
							...
							<li style="width:{$waterfallwidth}px">
								<!--{if !$_GET['archiveid'] && $_G['forum']['ismoderator']}-->
									<div style="position:absolute;margin:1px;padding:2px;">
									<!--{if $thread['fid'] == $_G[fid]}-->
										<!--{if $thread['displayorder'] <= 3 || $_G['adminid'] == 1}-->
											<input onclick="tmodclick(this)" type="checkbox" name="moderate[]" value="$thread[tid]" />
										<!--{else}-->
								...
								<div class="auth cl">
									<cite class="xg1 y">
										<!--{if $thread[recommends]}-->
										<em class="fico-thumbup fic10"></em>$thread[recommends]&nbsp;&nbsp;
										<!--{/if}-->
										<!--{if $thread[replies]}-->
										<em class="fico-comment fic10"></em><a href="forum.php?mod=viewthread&tid=$thread[tid]&extra=$extra" title="$thread[replies] {lang reply}">$thread[replies]</a>
										<!--{/if}-->
									</cite>
									<!--{hook/forumdisplay_author $key}-->
									<!--{if $thread['authorid'] && $thread['author']}-->
...
<div class="bw0 pgs cl">
	...
	<!--{if !$_GET['archiveid']}--><a href="javascript:;" id="newspecialtmp" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && empty($_G['forum']['threadsorts']['required'])} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} title="{lang send_posts}" class="pgsbtn showmenu" style="display:none;">{lang send_posts}</a><!--{/if}-->
