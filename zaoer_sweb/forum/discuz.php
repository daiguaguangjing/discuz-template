...
<!--{if empty($gid)}--><!--{ad/text/wp a_t}--><!--{/if}-->
...
<div id="ct" class="wp cl{if $_G['setting']['forumallowside']} ct2{/if}">
	<!--{if empty($gid)}-->
	<div class="zaochart zaobm bg0 p15 mb15 cl">
		<p class="z zaogrey">{lang index_today}: <em>$todayposts</em><span class="pipe">|</span>{lang index_yesterday}: <em>$postdata[0]</em><span class="pipe">|</span>{lang index_posts}: <em>$posts</em><span class="pipe">|</span>{lang index_members}: <em>$_G['cache']['userstats']['totalmembers']</em><!--{if $_G['cache']['userstats']['newsetuser']}--><span class="pipe">|</span>{lang welcome_new_members}: <em><a href="home.php?mod=space&username={echo rawurlencode($_G['cache']['userstats']['newsetuser'])}" target="_blank" class="zaoblue">$_G['cache']['userstats']['newsetuser']</a></em><!--{/if}--></p>
		<div class="y">
			<!--{hook/index_nav_extra}-->
			<!--{if $_G['setting']['forumallowside']}-->
			<!--{else}-->
			<a onclick="showWindow('nav', this.href, 'get', 0)" href="forum.php?mod=misc&action=nav" class="zaoblue">{lang i_want}{lang m_new_t}</a><span class="pipe">|</span>
			<!--{/if}-->
			<!--{if $_G['uid'] && $_G['setting']['forumstatus']}-->
			<a href="home.php?mod=space&do=thread&view=me" title="{lang my_posts}" class="zaoblue">{lang my_posts}</a>
			<!--{/if}-->
			<!--{if !empty($_G['setting']['search']['forum']['status'] && $_G['setting']['guidestatus'])}-->
			<!--{if $_G['uid'] && $_G['setting']['forumstatus']}--><span class="pipe">|</span><!--{/if}-->
			<a href="forum.php?mod=guide&view=new" title="{lang show_newthreads}" class="zaoblue">{lang show_newthreads}</a>
			<!--{/if}-->
		</div>
	</div>
	<!--{/if}-->
	...
	<div class="mn">
		...
		<!--[diy=sweb_mn_banner]--><div id="sweb_mn_banner" class="area"></div><!--[/diy]-->
		<script>
			var zaoSwiper = new Swiper ('.sweb_mn_banner', {
				loop: true,
				autoplay: {
					delay: 3500,
		...
		<!-- index four grid -->
		<div class="zaofourgrid mb15 zaobm bg0 cl">
			<div class="cl">
		...
		<div class="zaobm bg0 mb15">
			<div class="bm_h cl"><h2>{lang hotthreads_forum}</h2></div>
			<div class="bm_c cl">
				<div class="zaoheat z">
					<!--{loop $_G['cache']['heats']['message'] $data}-->
					<dl class="heatxld">
						<dt>
							<!--{if $_G['adminid'] == 1}--><a class="d fic15" href="forum.php?mod=misc&action=removeindexheats&tid=$data[tid]" onclick="return removeindexheats()"></a><!--{/if}-->
							<a href="forum.php?mod=viewthread&tid=$data[tid]" target="_blank" class="fic18">$data[subject]</a>
						</dt>
						<dd class="fic15 xg2">$data[message]</dd>
					</dl>
					<!--{/loop}-->
				</div>
				<div class="zaoheatl">
					...
					<dl class="heatxld">
						<dt>
							<!--{if $_G['adminid'] == 1}--><a class="d fic15" href="forum.php?mod=misc&action=removeindexheats&tid=$data[tid]" onclick="return removeindexheats()"></a><!--{/if}-->
							<a href="forum.php?mod=viewthread&tid=$data[tid]" target="_blank" class="fic15">$data[subject]</a>
						</dt>
					</dl>
					<!--{/loop}-->
				</div>
		...
		<div class="cl">
			...
			<div class="zaobm bg0 mb10{if $forumcolumns} flg{/if} cl follows">
				<div class="bm_h cl">
					<span class="o">
					...
					<table cellspacing="0" cellpadding="0" class="fl_tb">
						...
						<!--{loop $collectiondata['follows'] $key $colletion}-->
							...
							<td class="fl_g"{if $forumcolwidth} width="$forumcolwidth"{/if}>
								<div class="fliconmore">
									<a href="forum.php?mod=collection&action=view&ctid={$colletion[ctid]}" target="_blank" title="$colletion[name]">
										<!--{if $followcollections[$key]['lastvisit'] < $colletion['lastupdate']}-->
								...
								<dl>
									<dt class="fic18"><a href="forum.php?mod=collection&action=view&ctid={$colletion[ctid]}">$colletion[name]</a></dt>
									<dd class="xg2"><em>{lang forum_threads}:<!--{echo dnumber($colletion[threadnum])}--></em>&nbsp;&nbsp;<em>{lang collection_commentnum}:<!--{echo dnumber($colletion[commentnum])}--></em></dd>
									<dd class="xg2">
			...
			<!--{eval $forumcolwidth = (floor(100 / $forumcolumns) - 0.1).'%';}-->
			<div class="zaobm bg0 mb10{if $forumcolumns} flg{/if} cl fav">
				<div class="bm_h cl">
					<span class="o">
					...
					<table cellspacing="0" cellpadding="0" class="fl_tb">
						...
						<!--{eval $forumurl = !empty($forum['domain']) && !empty($_G['setting']['domain']['root']['forum']) ? $_G['scheme'].'://'.$forum['domain'].'.'.$_G['setting']['domain']['root']['forum'] : 'forum.php?mod=forumdisplay&fid='.$forum['fid'];}-->
							<!--{if $forumcolumns>1}-->
								...
								<!--{/if}-->
								<td class="fl_g"{if $forumcolwidth} width="$forumcolwidth"{/if}>
									<div class="fliconmore"{if !empty($forum[extra][iconwidth]) && !empty($forum[icon])} style="width:95px;"{/if}>
									<!--{if $forum[icon]}-->
										$forum[icon]
									<!--{else}-->
										<a href="$forumurl"{if $forum[redirect]} target="_blank"{/if} title="$forum[name]">
											<!--{if $forum[folder]}-->
											<img src="./template/zaoer_sweb/zaoer/forum-icon-light.png" />
											<!--{else}-->
											<img src="./template/zaoer_sweb/zaoer/forum-icon.png" />
									...
									<dl{if !empty($forum[extra][iconwidth]) && !empty($forum[icon])} style="margin-left:95px;"{/if}>
										<dt class="fic18"><a href="$forumurl"{if $forum[redirect]} target="_blank"{/if}{if $forum[extra][namecolor]} style="color: {$forum[extra][namecolor]};"{/if}>$forum[name]</a><!--{if $forum[todayposts] && !$forum['redirect']}--><em class="xw0 xi1" title="{lang forum_todayposts}"> ($forum[todayposts])</em><!--{/if}--></dt>
										<!--{if empty($forum[redirect])}-->
										<dd class="xg2"><em>{lang forum_threads}:<!--{echo dnumber($forum[threads])}--></em>&nbsp;&nbsp;<em>{lang forum_posts}:<!--{echo dnumber($forum[posts])}--></em></dd>
										<!--{/if}-->
										<dd class="xg2">
								...
								<!--{eval $favorderid++;}-->
							<!--{else}-->
								<td class="fliconone" style="width:95px;">
									<!--{if $forum[icon]}-->
										$forum[icon]
									<!--{else}-->
										<a href="$forumurl"{if $forum[redirect]} target="_blank"{/if} title="$forum[name]">
								...
								<td>
									<h2 class="fic18"><a href="$forumurl"{if $forum[redirect]} target="_blank"{/if}{if $forum[extra][namecolor]} style="color: {$forum[extra][namecolor]};"{/if}>$forum[name]</a><!--{if $forum[todayposts] && !$forum['redirect']}--><em class="xw0 xi1" title="{lang forum_todayposts}"> ($forum[todayposts])</em><!--{/if}--></h2>
									<!--{if $forum[description]}--><p class="xg2">$forum[description]</p><!--{/if}-->
									<!--{if $forumlist[$forum['fid']]['subforums']}--><p class="xg2">{lang forum_subforums}: $forumlist[$forum['fid']]['subforums']</p><!--{/if}-->
									<!--{if $forum['moderators']}--><p class="xg2">{lang forum_moderators}: <span class="xi2">$forum[moderators]</span></p><!--{/if}-->
									<!--{hook/index_favforum_extra $forum['fid']}-->
								</td>
								<td class="fl_i">
			...
			<div class="zaobm bg0 mb10{if $cat['forumcolumns']} flg{/if} cl loop">
				<div class="bm_h cl">
					<span class="o">
				...
				<div id="category_$cat[fid]" class="bm_c" style="{echo $collapse['category_'.$cat[fid]] or ''}">
					<table cellspacing="0" cellpadding="0" class="fl_tb">
						...
						<!--{else}-->
							...
							<td>
								<h2 class="fic18"><a href="$forumurl"{if $forum[redirect]} target="_blank"{/if}{if !empty($forum[extra][namecolor])} style="color: {$forum[extra][namecolor]};"{/if}>$forum[name]</a><!--{if $forum[todayposts] && !$forum['redirect']}--><em class="xw0 xi1" title="{lang forum_todayposts}"> ($forum[todayposts])</em><!--{/if}--></h2>
								<!--{if $forum[description]}--><p class="xg2">$forum[description]</p><!--{/if}-->
								<!--{if $forum['subforums']}--><p class="xg2">{lang forum_subforums}: $forum['subforums']</p><!--{/if}-->
								<!--{if $forum['moderators']}--><p>{lang forum_moderators}: <span class="xi2">$forum[moderators]</span></p><!--{/if}-->
								<!--{hook/index_forum_extra $forum['fid']}-->
							</td>
							<td class="fl_i">
								<!--{if empty($forum[redirect])}--><span class="xi2"><!--{echo dnumber($forum[threads])}--></span><span class="xg2"> / <!--{echo dnumber($forum[posts])}--></span><!--{/if}-->
							</td>
							<td class="fl_by">
								<div class="xg2">
								<!--{if $forum['permission'] == 1}-->
			...
			<!--{eval $forumcolumns = 4;}-->
			<!--{eval $forumcolwidth = (floor(100 / $forumcolumns) - 0.1).'%';}-->
			<div class="zaobm bg0 mb10{if $forumcolumns} flg{/if} cl collection">
				<div class="bm_h cl">
					<span class="o">
		...
		<!--{if empty($gid) && $_G['setting']['whosonlinestatus']}-->
			<div id="online" class="zaobm bg0 mb10 oll">
				<div class="bm_h">
				<!--{if $detailstatus}-->
		...
		<div class="zaobm bg0 p15 mb10 lk">
			<div id="category_lk" class="cl">
	...
	<div id="sd" class="sd">
		<!--{hook/index_side_top}-->
		<!--[diy=sweb_sd_one]--><div id="sweb_sd_one" class="area"></div><!--[/diy]-->
		<!--[diy=sweb_sd_two]--><div id="sweb_sd_two" class="area"></div><!--[/diy]-->
		<!--[diy=sweb_sd_three]--><div id="sweb_sd_three" class="area"></div><!--[/diy]-->
		<!--[diy=sweb_sd_four]--><div id="sweb_sd_four" class="area"></div><!--[/diy]-->
		<!--[diy=sweb_sd_five]--><div id="sweb_sd_five" class="area"></div><!--[/diy]-->
		<!--[diy=sweb_sd_six]--><div id="sweb_sd_six" class="area"></div><!--[/diy]-->
