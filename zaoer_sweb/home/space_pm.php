...
<div id="ct" class="ct2_a wp cl">
	<div class="mn">
		<div class="bm bw0">
			<h1 class="mt"><i class="fico-email fic4 fc-p vm"></i>{lang news}</h1>
			<ul class="tb cl">
					...
					<div class="tbmu pml pm_op_r cl">
						<div class="y pm_o">
							...
							<div id="pm_operation_menu" class="p_pop" style="display: none;">
								<ul>
									<li><a href="home.php?mod=spacecp&ac=pm&op=delete&deletepm_delplid[]=$plid&plid=$plid&handlekey=pmdeletehk_{$plid}&formhash={FORMHASH}" id="a_pmdelete_$plid" onclick="showWindow(this.id, this.href, 'get', 0);" title="{lang delete_privatepm_user}">{lang pm_delete_all}</a></li>
								</ul>
							</div>
						</div>
				...
				<!--{elseif $touid}-->
					<div class="tbmu pml pm_op_r cl">
						<div class="y pm_o">
							...
							<div id="pm_operation_menu" class="p_pop" style="display: none;">
								<ul>
									<li><a href="home.php?mod=spacecp&ac=pm&op=delete&deletepm_deluid[]=$touid&uid=$touid&handlekey=pmdeletehk_{$plid}&formhash={FORMHASH}" id="a_pmdelete_$plid" onclick="showWindow(this.id, this.href, 'get', 0);" title="{lang delete_privatepm_user}">{lang pm_delete_all}</a></li>
								</ul>
							</div>
						</div>
						<div class="xw1">
						...
						<div class="y pm_o">
							...
							<div id="pm_operation_menu" class="p_pop" style="display: none;">
								<ul>
									...
									<li><a href="home.php?mod=spacecp&ac=pm&op=delete&deletepm_delplid[]=$plid&plid=$plid&handlekey=pmdeletehk_{$plid}&formhash={FORMHASH}" id="a_pmdelete_$plid" onclick="showWindow(this.id, this.href, 'get', 0);" title="{lang delete_chatpm_comment}">{lang delete_chatpm}</a></li>
									<!--{else}-->
									<li><a href="home.php?mod=spacecp&ac=pm&op=delete&deletepm_quitplid[]=$plid&plid=$plid&handlekey=pmdeletehk_{$plid}&formhash={FORMHASH}" id="a_pmdelete_$plid" onclick="showWindow(this.id, this.href, 'get', 0);" title="{lang quit_chatpm_comment}">{lang quit_chatpm}</a></li>
									<!--{/if}-->
								</ul>
							</div>
						</div>
							...
							<dd class="ptm">
								<form id="pmform" name="pmform" method="post" autocomplete="off" action="home.php?mod=spacecp&ac=pm&op=send&pmid=$pmid&daterange=$daterange&handlekey=pmsend&pmsubmit=yes" onsubmit="this.message.value = parseurl(this.message.value);ajaxpost('pmform', 'pmforum_return', 'pmforum_return');return false;">
									<div class="tedt">
										<div class="bar">
											<!--{if $list && $daterange && !$touid}-->
											<span onclick="refreshMsg(1);" title="{lang refresh}" class="y xg1 cur1"><i class="fico-refresh fic4 vm" title="{lang refresh}"></i><span id="refreshtip">{lang refresh}</span></span>
											<!--{/if}-->
						...
						<dl class="bbda cl">
							<dd class="y mtm pm_o">
								...
								<div id="pm_o_$grouppm[id]_menu" class="p_pop" style="display: none;">
									<ul>
										<li><a href="home.php?mod=spacecp&ac=pm&op=delete&deletepm_gpmid[]=$grouppm[id]&pmid=$grouppm[id]&handlekey=gpmdeletehk_{$grouppm[id]}&formhash={FORMHASH}" id="a_gpmdelete_$grouppm[id]" onclick="showWindow(this.id, this.href, 'get', 0);" title="{lang delete}">{lang delete}</a></li>
									</ul>
								</div>
							</dd>
							<dd class="m avt">
				...
				<form id="pmsettingform" name="pmsettingform" method="post" autocomplete="off" action="home.php?mod=spacecp&ac=pm&op=setting">
					<table cellspacing="0" cellpadding="0" class="tfm mtm">
						...
						<tr>
							...
							<td>
								<textarea id="ignorelist" name="ignorelist" cols="40" rows="3" class="pt" onkeydown="ctrlEnter(event, 'ignoresubmit');">{echo htmlspecialchars($ignorelist)}</textarea>
							...
							<!--{if $grouppms}-->
								<!--{loop $grouppms $grouppm}-->
									<dl id="gpmlist_$grouppm[id]" class="bbda cur1 cl{if !$gpmstatus[$grouppm[id]]} newpm{/if}">
										<dd class="y mtm pm_o">
											<a href="javascript:;" id="pm_g_$grouppm[id]" class="o" onmouseover="showMenu({'ctrlid':this.id, 'pos':'34'})">{lang menu}</a>
											<div id="pm_g_$grouppm[id]_menu" class="p_pop" style="display: none;">
												<ul>
													<li><a href="javascript:;" onclick="ajaxget('home.php?mod=spacecp&ac=pm&op=delete&deletesubmit=1&deletepm_gpmid[]=$grouppm[id]&formhash={FORMHASH}', '', 'ajaxwaitid', '', 'none', '$(\'gpmlist_$grouppm[id]\').style.display=\'none\';');">{lang delete}</a></li>
												</ul>
											</div>
										</dd>
										<dd class="m avt">
									...
									<dd class="ptm pm_c">
										...
										<!--{if $value['pmtype'] == 1}-->
											...
											<span class="pm_o y">
												<div id="pm_o_$value[plid]_menu" class="p_pop" style="display: none;">
													<ul>
														<!--{if $value['pmtype'] == 1}-->
														<li><a href="javascript:;" onclick="ajaxget('home.php?mod=spacecp&ac=pm&op=delete&deletesubmit=1&deletepm_deluid[]=$value[touid]&formhash={FORMHASH}', '', 'ajaxwaitid', '', 'none', '$(\'pmlist_$value[plid]\').style.display=\'none\';');">{lang delete}</a></li>
														...
														<!--{elseif $value['pmtype'] == 2}-->
															<!--{if $value['authorid'] == $_G['uid']}-->
														<li><a href="javascript:;" onclick="ajaxget('home.php?mod=spacecp&ac=pm&op=delete&deletesubmit=1&deletepm_delplid[]=$value[plid]&formhash={FORMHASH}', '', 'ajaxwaitid', '', 'none', '$(\'pmlist_$value[plid]\').style.display=\'none\';');">{lang delete}</a></li>
															<!--{else}-->
														<li><a href="javascript:;" onclick="ajaxget('home.php?mod=spacecp&ac=pm&op=delete&deletesubmit=1&deletepm_quitplid[]=$value[plid]&formhash={FORMHASH}', '', 'ajaxwaitid', '', 'none', '$(\'pmlist_$value[plid]\').style.display=\'none\';');">{lang delete}</a></li>
															<!--{/if}-->
														<!--{/if}-->
													</ul>
												</div>
										...
										<!--{elseif $value['pmtype'] == 2}-->
											...
											<span class="pm_o y" style="margin-top: -20px; ">
												...
												<a href="javascript:;" id="pm_o_$value[plid]" class="o" onmouseover="showMenu({'ctrlid':this.id, 'pos':'34'})">{lang menu}</a>
												 | 
												<a href="home.php?mod=space&do=pm&subop=view&plid=$value[plid]&type=1&daterange=$value[daterange]#last" id="pmlist_$value[plid]_a">{lang reply}</a>
											</span>
										<!--{/if}-->
									</dd>
								</dl>
