		...
		<!--{if in_array($do, array('feed', 'view'))}-->
			<!--{if helper_access::check_module('follow') && ( $do == 'feed' || ( $do == 'view' && $viewself))}-->
				<div id="flw_header" class="mbw bbs cl">
					...
					<form method="post" autocomplete="off" id="fastpostform" action="home.php?mod=spacecp&ac=follow&op=newthread&topicsubmit=yes&infloat=yes&handlekey=fastnewpost&inajax=1" onsubmit="return fastpostvalidate(this);" style="float: left;">
						...
						<div id="flw_post_extra" class="mtn cl">
							<div{if $_G[setting][fastsmilies]} class="hasfsl"{/if} id="fastposteditor">
								<div class="tedt">
									<div class="area">
										<textarea rows="5" cols="80" name="message" id="fastpostmessage" onKeyDown="seditor_ctlent(event, '$(\'fastpostsubmit\').click()');" class="pt xs2"></textarea>
									</div>
							...
							<div class="cl">
								...
								<div class="y">
									<p class="ptm pnpost">
										...
										<button {if $_G['uid']}type="submit" {else}type="button" onclick="showWindow('login', 'member.php?mod=logging&action=login&guestmessage=yes')" {/if}name="topicsubmit_btn" id="fastpostsubmit" value="topicsubmitbtn" class="pgsbtn"><strong>{lang follow_new_thread}</strong></button>
									</p>
								</div>
							</div>
						</div>
