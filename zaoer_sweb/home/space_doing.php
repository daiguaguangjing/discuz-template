		...
		<!--{if $dolist}-->
			...
			<!--{eval $_GET['key'] = $key = random(8);}-->
				<dl id="{$key}dl{$doid}" class="pbn bbda cl">
					<!--{if empty($diymode)}--><dd class="m avt"><a href="home.php?mod=space&uid=$dv[uid]" c="1"><!--{avatar($dv['uid'], 'middle')}--></a></dd><!--{/if}-->
					...
					<dd class="ptn xg1">
						...
						<!--{if $dv[uid]==$_G[uid]}-->
						<span class="pipe">|</span><a href="home.php?mod=spacecp&ac=doing&op=delete&doid=$doid&id=$dv[id]&handlekey=doinghk_{$doid}_$dv[id]" id="{$key}_doing_delete_{$doid}_{$dv[id]}" onclick="showWindow(this.id, this.href, 'get', 0);">{lang delete}</a>
						<!--{/if}-->
						
						<!--{if checkperm('managedoing')}-->
						<span class="pipe" style="display: none;">IP: $dv[ip]:$dv[port]</span>
						<!--{/if}-->
					</dd>
				</dl>
			<!--{/loop}-->
