...
<!--{eval
	$space_uid = $space['uid'];
	$_G['home_tpl_spacemenus'][] = "<a href=\"home.php?mod=space&uid=$space_uid&do=activity&view=me\">{lang they_activity}</a>";
}-->
...
<div id="pt" class="bm cl">
	<div class="z">
		...
		<a href="home.php?mod=space&do=thread">{lang post}</a> <em>&rsaquo;</em>
	 	<a href="home.php?mod=space&uid=$space[uid]&do=activity&view=me">{lang activity}</a>
	</div>
</div>
<!--{eval 
...
<div id="ct" class="ct2_a wp cl">
	<div class="mn">
		...
		<!--{if (!$_G['uid'] && !$space[uid]) || $space[self]}-->
			<h1 class="mt"><i class="fico-group fic4 fc-p vm"></i>{lang activity}</h1>
		<!--{/if}-->
		<!--{if $space[self]}-->
			<ul class="tb cl">
		...
		<!--{if $list}-->
			...
			<table cellpadding="0" cellspacing="0" class="acl mbm">
				...
				<!--{/if}-->
				<tr>
					<td class="type"><!--{if $thread[aid]}--><img src="{echo getforumimg($thread[aid])}" alt="$thread[subject]" width="80" /><!--{else}--><div class="nophoto" title="nophoto"></div><!--{/if}--></td>
					<td>
